...
> **Update 2026-09-08.** Острый обрыв Tailscale-egress (раздел «link 1
> сейчас мёртв») прошёл: `tailscale status` показывает `oci-pve` с
> **прямым** путём (`active; direct …:41641`), домашняя сторона снова
> ходит через DERP (`fra`). Кольцо corosync на нодах повторно не
> проверялось — если `pvecm status` всё ещё `Nodes: 1`, скорее всего
> достаточно `systemctl restart corosync` на `oci-pve` (линк был down,
> knet сам не всегда переподключается). **Структурная часть TODO
> (link 0 на недостижимых LAN-адресах, votes=4 на пороге кворума,
> WAN-таймауты) остаётся в силе.**
>
> Отдельно: эта нода больше не Tailscale subnet-роутер для
> `container_subnet` — теперь она **app-connector** для группы
> `smart-vpn-users` (см. `tailscale-acl`, `mod/oci-pve-node/cloud-init/bootstrap.sh.tpl`).
> К вступлению в кластер это отношения не имеет, но `--advertise-routes`
> из `tailscale up` убран — если для corosync-линка когда-то
> понадобится маршрут, добавлять его отдельно.
